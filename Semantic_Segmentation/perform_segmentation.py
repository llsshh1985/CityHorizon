...
if __name__ == '__main__':
    ...
    files=open("/data/yyh/CityWander/Semantic_Segmentation/test_img.txt","r")
    files=files.read().split("\n")
    files=fil(files)

    for i in files:

        args.input_image="/data/yyh/CityWander/Semantic_Segmentation/test_Beijing/"+i
	print(args.input_image)
        input_image = cv2.imread(args.input_image, 1).astype(np.float32)
	input_image = input_image[0:600,0:960]
        ...
        if args.out_dir is not None:
            ...
            args.out_dir="/data/yyh/CityWander/Semantic_Segmentation/test_rgb_data/"
	    
	   # print(args.out_dir)

            out_path_im = args.out_dir + input_image_name + '_enet' + '.' + input_path_ext
            cv2.imwrite(out_path_im, prediction_rgb)
	    #rgb_data=open(args.out_dir + input_image_name + '_rgb' + '.' + "txt","w")	
	    #np.savetxt(args.out_dir + input_image_name + '_rgb' + '.' + "txt",prediction_rgb)    
	#rgb_data.write(prediction_rgb)    
            
            args.out_dir="/data/yyh/CityWander/Semantic_Segmentation/test_label_data/"
	    ...
	    #label_data=open(args.out_dir + input_image_name + '_label' + '.' + "txt","w")         
            #label_data.write(prediction_rgb)  
	    #np.savetxt(args.out_dir + input_image_name + '_label' + '.' + "txt",prediction)
